  ...
  try {
    ...
    if (response.code === 0 && response.data) {
      ...
      const pieData = frequencyRanges.map(range => ({
        name: range.name,
        value: data.filter((item: { value: number }) => item.value >= range.min && item.value < range.max).length
      }))
      ...
      const lineOption = {
        ...
        series: top5Words.map(word => ({
          ...
          data: Array(7).fill(null).map(() => {
            const wordItem = data.find((item: { name: string; value: number }) => item.name === word)
            const value = wordItem?.value ?? 0
            return Math.floor(value * Math.random() * 0.5 + value * 0.5)
          })
        }))
      }
